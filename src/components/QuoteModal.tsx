...
const QuoteModal: React.FC<QuoteModalProps> = ({ productId, productName, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await apiRequest('/quote', { method: 'POST', body: { productId, productName, ...form } });
      setStatus('success');
    } catch (err) {
      setStatus('error');
      ...
      >
        ...
        {status === 'success' ? (
          <div className="text-center py-6">
            <p className="text-emerald-400 text-sm font-medium">Quote request sent.</p>
            <p className="text-white/60 text-sm mt-2">
              Our team will follow up about <span className="text-white">{productName}</span> shortly.
            </p>
            <button
              onClick={onClose}
          ...
          <>
            <h3 className="text-white text-lg font-playfair italic">Request a quote</h3>
            <p className="text-white/50 text-xs mt-1">For {productName}</p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-3">
              ...
              <input name="company" type="text" placeholder="Company Name" value={form.company} onChange={handleChange} className={inputClasses} />
              <textarea name="message" placeholder="Quantity / details" rows={3} value={form.message} onChange={handleChange} className={`${inputClasses} resize-none`} />
